...
**Developer Friendly** — Intuitive functional API that feels natural to write and easy to read.
...
```bash
pnpm add @player-lang/functional-dsl
```

### Your First Flow

```typescript
import { flow, binding as b, expression as e } from "@player-lang/functional-dsl";
...
```typescript
import { binding as b, expression as e } from "@player-lang/functional-dsl";
...
Functional comes with a code generator that creates type-safe builders for your custom Player-UI assets.

### Setup

The Functional DSL includes a code generator (`@player-lang/functional-dsl-generator`) that creates type-safe builders from your TypeScript asset types.
...
for (const assetType of result.data.types) {
  ...
  const code = generateFunctionalBuilder(assetType, outputDir, config);
...
import { text, binding as b } from "@player-lang/functional-dsl";
...
import { extractBindingsFromSchema } from "@player-lang/functional-dsl";
...
```typescript
import { and, or, greaterThan, lessThan, equal } from "@player-lang/functional-dsl";
...
```typescript
import { globalIdRegistry } from "@player-lang/functional-dsl";
...
The Functional DSL provides powerful conditional methods that let you set properties based on runtime conditions during the build phase. Unlike switches (which create runtime conditional rendering), these methods determine asset structure at build time.
...
```typescript
import { collection, text, template, binding as b } from "@player-lang/functional-dsl";
...
```typescript
import { BaseBuildContext } from "@player-lang/functional-dsl";
...
import { binding, expression } from "@player-lang/functional-dsl";

// Flow creation
import { flow } from "@player-lang/functional-dsl";
