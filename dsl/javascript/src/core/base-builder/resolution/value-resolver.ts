 ...
 * - FunctionalBuilder/AssetWrapper → passed through unchanged (resolved later)
...
export function extractValue(
  ...
  // But skip FunctionalBuilders and AssetWrappers (they'll be resolved later)
  if (
    isPlainObject(value) &&
    !isFunctionalBuilder(value) &&
...
/**
 * Resolves a value, handling FunctionalBuilders, AssetWrappers, and nested structures
 */
export function resolveValue<T, C extends BaseBuildContext>(
  ...
  // Handle FunctionalBuilder instances
  if (isFunctionalBuilder<T, C>(value)) {
...
export function resolveAndWrapAsset<C extends BaseBuildContext>(
  ...
  // If it's a FunctionalBuilder, we need to call it with a context that has a slot branch
