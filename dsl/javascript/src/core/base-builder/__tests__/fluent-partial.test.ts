...
import { text, collection, action } from "../../../__mocks__/generated";
import { resetGlobalIdSet } from "../";
import { binding as b } from "../../tagged-template";

describe("FunctionalPartial - Constructor with TaggedTemplateValue", () => {
  beforeEach(() => {
    resetGlobalIdSet();
  });
  ...
  test("collection() accepts FunctionalBuilder in constructor for label property", () => {
  ...
  test("collection() accepts array of FunctionalBuilders in constructor for values property", () => {
    const collectionAsset = collection({
