...
import type { FunctionalBuilder, BaseBuildContext } from "../types";
import { isFunctionalBuilder, isAsset, isAssetWrapperValue } from "../guards";
...
export function resolveValueOrFunction<V>(value: V | (() => V)): V {
  if (typeof value === "function" && !isFunctionalBuilder(value)) {
    // SAFETY: We've checked it's a function and not a FunctionalBuilder,
...
export function shouldWrapInAssetWrapper(
  ...
  | FunctionalBuilder<unknown, BaseBuildContext>
  | Asset
  | Array<FunctionalBuilder<unknown, BaseBuildContext> | Asset> {
  // Don't wrap if already wrapped (has 'asset' property but not 'type')
  if (isAssetWrapperValue(value)) {
    return false;
  }

  // Wrap FunctionalBuilders
  if (isFunctionalBuilder(value)) {
    return true;
  }
  ...
  if (Array.isArray(value) && value.length > 0) {
    ...
    return isFunctionalBuilder(firstItem) || isAsset(firstItem);
  }
