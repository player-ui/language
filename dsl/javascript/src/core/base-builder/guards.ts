 ...
 */
export function isFunctionalBuilder<
  T = unknown,
  C extends BaseBuildContext = BaseBuildContext,
>(value: unknown): value is FunctionalBuilder<T, C> {
...
/**
 * Type guard to check if a value is an array of FunctionalBuilders
 */
export function isBuilderArray<
  T = unknown,
  C extends BaseBuildContext = BaseBuildContext,
>(value: unknown): value is Array<FunctionalBuilder<T, C>> {
  return Array.isArray(value) && value.every(isFunctionalBuilder);
}
...
export function needsAssetWrapper(value: unknown): boolean {
  ...
  // Don't wrap if it's a FunctionalBuilder (will be resolved later)
  if (isFunctionalBuilder(value)) return false;
