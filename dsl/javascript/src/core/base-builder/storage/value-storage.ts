...
import { isFunctionalBuilder } from "../guards";
 ...
 * - builders: FunctionalBuilder instances and objects containing builders
...
export class ValueStorage<T> {
  ...
  private builders: Map<
    string,
    FunctionalBuilder<unknown, BaseBuildContext> | Record<string, unknown>
  > = new Map();
   ...
   * - FunctionalBuilder instances → builders Map
  ...
  private containsBuilder(
    ...
    if (isFunctionalBuilder(obj)) return true;
  ...
  peekBuilder<K extends keyof T, C extends BaseBuildContext>(
    key: K,
  ): FunctionalBuilder<T[K], C> | undefined {
    const keyStr = String(key);
    const entry = this.builders.get(keyStr);
    if (!entry) return undefined;

    if (isFunctionalBuilder<T[K], C>(entry)) {
      return entry;
    }
